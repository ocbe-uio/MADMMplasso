...
theta[1,1,1] <- 2; theta[3,2,1] <- 2;
theta[1,1,2] <- 2; theta[3,2,2] <- 2;
theta[6,1,3] <- 2; theta[8,2,3] <- 2;
theta[6,1,4] <- 2; theta[8,2,4] <- 2;
theta[11,1,5] <- 2; theta[13,2,5] <- 2;
theta[11,1,6] <- 2; theta[13,2,6] <- 2
...
system.time(fit <- MADMMplasso(
    X=X, Z=Z, y=as.matrix(y),
    alpha = alpha, my_lambda = NULL,
    lambda_min = 0.001, max_it = 1000, e.abs = e.abs, e.rel = e.rel,
    maxgrid = nlambda, nlambda = nlambda, rho = 5, tree = TT, my_print = T,
    alph = TRUE, gg = gg1, tol = tol, cl = 2L,legacy = F
) )
